...
进了决赛，去了重庆（见[龙芯杯三日游](/blog/diary/龙芯杯三日游/)），答辩过后依旧兴奋，直到第二天的颁奖典礼过后才冷静下来。拿了二等奖，破了学校记录，还是非常心满意足的。比赛一年比一年难，从最开始的完成即完赛，到现在标配 cache，不敢想象过两年是不是要标配多发射了。不过我能做的，就是把参赛的 CPU 写好注释，写好文档，让他可以传承下去，让更多人能够学会设计 CPU，调试 CPU。
...
1. 获取状态 $$q$$；
2. 将状态，输入二元组映射到三元组： $$\delta(q, Z) \mapsto (p, Y, D)$$；
3. 转移状态，并回到1。

即，**取指令**，**译码（映射到操作）**，和**执行**等三部分。

事实上，编写 brainfuck 语言的代码便是人肉模拟图灵机，理论上可以与其他高级语言做到同样的事（相信你不会想这么做的），比如[这里](https://github.com/AllAlgorithms/brainfuck/blob/master/algorithms/math/fibonacci.bf)便是一个计算斐波那契数列的程序。
...
![top structure]({{ '/assets/images/nscscc/nscscc_simple_core.webp' | relative_url }})
...
![if module structure]({{ '/assets/images/nscscc/nscscc_simple_if.webp' | relative_url }})
...
![id module structure]({{ '/assets/images/nscscc/nscscc_simple_id.webp' | relative_url }})
...
![exe module structure]({{ '/assets/images/nscscc/nscscc_simple_exe.webp' | relative_url }})
...
![pipeline CPU]({{ '/assets/images/nscscc/nscscc_pipeline_f1.webp' | relative_url }})
...
![pipeline CPU]({{ '/assets/images/nscscc/nscscc_pipeline_f2.webp' | relative_url }})

假设我们每间隔 $$D/n+S$$ 便向处理器中输入一条指令，每一级部件都可以完成自己的工作，并按时送到两级部件之间的寄存器中，这样第一条指令的执行时间还保持 $$n(D/n+S)$$ 不变，但随后每隔 $$D/n+S$$ 便会有一条指令执行完毕。这个处理器的性能为

$$\text{Performance} = \frac{1/n(D/n+S) + (I-1)/(D/n+S)}{I}$$

其中 $$I$$ 是执行的指令数。当 $$I\to \infty$$ 时，有

$$\begin{aligned}
    \text{Performance} &= \lim_{I \to \infty}\frac{1/n(D/n+S) + (I-1)/(D/n+S)}{I} \\
    &= (D/n+S)\lim_{I \to \infty}\left(\frac{I+n-1}{I}\right) \\
    &= 1/(D/n + S)
\end{aligned} $$

当 $$S$$ 足够小时，几乎是单周期处理器性能的 $$n$$ 倍。

![pipeline CPU]({{ '/assets/images/nscscc/nscscc_pipeline_f3.webp' | relative_url }})
